// Loja — sementes/insumos mais baratos que o automático, silo e máquinas
window.MarketPanel = (function(){
  const CF = window.CF;
  const S = ()=> window.CF.state;

  const btn = (txt, attrs='')=> `<button class="btn" ${attrs}>${txt}</button>`;
  const row = (label, right='', btnHtml='')=> `<div class="row"><label>${label}</label><div>${right} ${btnHtml}</div></div>`;
  ...
  function renderSeeds(host){
    ...
    wrap.addEventListener('click', (e)=>{
      ...
      const price=CF.seedShopPrice(key);
      ...
      CF.render && CF.render(); CF.saveSlot && CF.saveSlot(st.playerName);
  ...
  function renderStorage(host){
    ...
    wrap.querySelector('#buySilo').addEventListener('click', ()=>{
      ...
      CF.buyItem('silo'); CF.saveSlot && CF.saveSlot(st.playerName);
  ...
  function renderMachines(host){
    ...
    order.forEach(k=>{
      const st=S(); const have=st ? st.inventory[k]>0 : false;
